...
export async function generateProductDescription(imageData: ImageData, productName: string, productTypeKey: string, messageId: string, processId: string): Promise<string> {
    try {
        logger.info(`Process ID: ${processId} | Message ID: ${messageId} - sending prompt to Generative AI for description generation`);
        ...
        **Key Features:**  
        ...
        logger.info(`Process ID: ${processId} | Message ID: ${messageId} - description generated`);
        return generatedDescription;

    } catch (error: any) {
        logger.error(`Process ID: ${processId} | Message ID: ${messageId} - error during generating description`, { 
