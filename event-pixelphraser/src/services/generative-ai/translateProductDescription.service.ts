...
export async function translateProductDescription(generatedDescription: string, languagesForTranslation: string[], messageId: string, processId: string): Promise<Record<string, string>> {
    try {
        logger.info(`Process ID: ${processId} | Message ID: ${messageId} - sending prompt to Generative AI for translation`);
        ...
        for (const language of languagesForTranslation) {
            ...
            **Translation Guidelines**:
            ...
            logger.info(`Process ID: ${processId} | Message ID: ${messageId} - translated to ${language}`);
        }

        return translations;
    } catch (error: any) {
        logger.error(`Process ID: ${processId} | Message ID: ${messageId} - error during translation`, { 
